    ...
    def start_listening(self):
        ...
        if IS_WINDOWS:
            ...
            if msvcrt is not None and sys.stdin.isatty():
                threading.Thread(target=self._listen_loop_windows, daemon=True).start()
            return
        try:
...
DANGEROUS_PATTERNS = [r'\brm\b', r'\bdel\b', r'\berase\b', r'\brmdir\b', r'\bRemove-Item\b', r'\bmv\b', r'\bsudo\b', r'\bchmod\b', r'\bchown\b', r'\bdd\b', r'\bmkfs\b', r'\bformat\b', r'\bkill\b', r'>\s*/dev/', r'\bshred\b', r'\bwipe\b']
ALLOWED_SEARCH_DOMAINS = ["ollama.com", "googleblog.com", "ai.google.dev", "huggingface.co"]
CLI_COMMAND_NAME = "OCLI.bat" if IS_WINDOWS else "OCLI"
CLI_INSTALL_PATHS = [] if IS_WINDOWS else [
    "/usr/local/bin/OCLI",
...
PIPE_READER_DONE = object()

def start_pipe_reader(stream):
    ...
    def reader():
        try:
            ...
            output_queue.put(PIPE_READER_DONE)

    threading.Thread(target=reader, daemon=True).start()
    return output_queue

def get_queued_output(output_queue, timeout=0.0):
    try:
...
def direct_command_from_user(text):
    ...
    if IS_WINDOWS:
        allowed.update({'py', 'dir', 'type', 'where', 'cmd', 'powershell', 'pwsh', 'copy', 'xcopy', 'robocopy'})
    return command if first in allowed else None

def hardware_inspection_command():
    if IS_WINDOWS:
        return 'powershell -NoProfile -Command "Get-CimInstance Win32_VideoController | Select-Object Name,AdapterRAM; Get-CimInstance Win32_ComputerSystem | Select-Object Model,TotalPhysicalMemory"'
    return "system_profiler SPDisplaysDataType SPHardwareDataType | sed -n '/Chipset Model/p;/VRAM/p;/Total Number of Cores/p;/Memory:/p;/Model Name/p;/Chip/p'"
...
class OCLI:
    def __init__(self, model_name, auto_mode=False, backend='ollama', url=None):
        ...
        platform_note = "Windows compatibility mode is active; prefer Python APIs or Windows-compatible commands such as dir, type, where, powershell, py, python, and pytest. " if IS_WINDOWS else ""
        self.messages = [{
            'role': 'system',
            'content': (
    ...
    def ensure_mlx_server(self):
        if IS_WINDOWS:
            log_info("MLX backend is Apple Silicon/macOS-only; choose ollama, llama-cpp, or airllm on Windows.")
            return
    ...
    def run_cmd(self, command):
        try:
            ...
            last_out = time.time()
            if IS_WINDOWS:
                output_queue = start_pipe_reader(process.stdout)
                reader_done = False
                while True:
                    ...
                    item = get_queued_output(output_queue, timeout=0.2)
                    if item is PIPE_READER_DONE:
                        reader_done = True
                    elif item:
                        print_frame_text(item)
                        output.append(item)
                        last_out = time.time()
                        continue
    ...
    def test_cmd(self, command):
        try:
            ...
            if IS_WINDOWS:
                ...
                self.active_output_queue = start_pipe_reader(self.active_process.stdout)
                output = []
                ...
                while True:
                    ...
                    item = get_queued_output(self.active_output_queue, timeout=0.1)
                    if item is PIPE_READER_DONE:
                        reader_done = True
                    elif item:
                        print_frame_text(item)
                        output.append(item)
                        last_output_time = time.time()
                    if reader_done and self.active_process.poll() is not None:
                        break
    ...
    def send_input(self, text):
        try:
            if IS_WINDOWS:
                if not self.active_process or self.active_process.poll() is not None or self.active_process.stdin is None:
                ...
                time.sleep(0.5)
                output = []
                if self.active_output_queue is not None:
                    while True:
                        item = get_queued_output(self.active_output_queue, timeout=0.1)
                        if item is PIPE_READER_DONE:
                            self.active_output_queue = None
    ...
    def search_files(self, query, path="."):
        try:
            ...
            print(f"\n  {frame_title('FILE SEARCH', Colors.MAGENTA)}\n{render_text(result)}\n  {frame_bottom(Colors.MAGENTA)}\n")
            return result
        except Exception as e: return f"Error: {str(e)}"
    ...
    def grep(self, pattern, path="."):
        try:
            ...
            matches = []
            for file_path in candidate_files(path or "."):
                try:
            ...
            print(f"\n  {frame_title('GREP RESULTS', Colors.MAGENTA)}\n{render_text(result)}\n  {frame_bottom(Colors.MAGENTA)}\n")
            return result
        except Exception as e: return f"Error: {str(e)}"
    ...
    def setup_llama_cpp(self):
        if IS_WINDOWS:
            log_info("Automated llama.cpp setup is only configured for macOS in this script.")
            log_info("On Windows, install CMake and Visual Studio Build Tools, then run llama.cpp's Windows build steps or use Ollama.")
            return "Setup skipped: automated llama.cpp setup is not available on Windows."
    ...
    def setup_mlx(self):
        if IS_WINDOWS:
            log_info("MLX is Apple Silicon/macOS-only. Use the ollama, llama-cpp, or airllm backend on Windows.")
            return "Setup skipped: MLX is not available on Windows."
            ...
            try:
                ...
                if not tool_calls and not had_tool_calls and direct_command:
                    ...
                    tool_calls.append({
                        'function': {
                            'name': 'run_cmd',
                            'arguments': {
                                'command': hardware_inspection_command()
                            }
                        }
                    })
                    ...
                    try:
                        ...
                        if not name or name not in available_tools:
                            ...
                            if any(term in self.last_user_goal.lower() for term in ['vram', 'gpu memory', 'video memory', 'unified memory', 'how much memory']):
                                ...
                                name = 'run_cmd'
                                args = {'command': hardware_inspection_command()}
                            else:
                                continue
                ...
                if valid_tool_calls == 0:
                    if any(term in self.last_user_goal.lower() for term in ['vram', 'gpu memory', 'video memory', 'unified memory', 'how much memory']):
                        ...
                        result = truncate_output(self.run_cmd(hardware_inspection_command()))
